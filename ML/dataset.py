...
class Data():
    ...
    def getdata(self):
        ...
        train, test, val = random_split(self.folderdata,[self.trainset, self.testset, self.valset], 
                                        generator=torch.Generator().manual_seed(42))
        
        trainloader = DataLoader(train, shuffle=True, batch_size=self.batch_size,
                                 num_workers=2)
        testloader =   DataLoader(test, shuffle=True, batch_size=self.batch_size,
                                 num_workers=2)      

        valloader = DataLoader(val, shuffle=True, batch_size=self.batch_size,
                                 num_workers=2)                                                          
